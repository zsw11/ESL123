...
<script>
import { fetch{{upperFirst modelName}}, update{{upperFirst modelName}} } from '@/api/{{modelName}}'
...
{{/compare}}{{/forOf}}      }
  ...
  created() {
    {{#forOf attrs}}{{#compare ../selectedAttrs 'includes' k}}{{#switch v.UIType}}{{#case 'remoteDict'}}this.get{{upperFirstRmId k}}s()
    {{/case}}{{/switch}}{{/compare}}{{/forOf}}this.getDetail(parseInt(this.$route.params.id))
  },
  methods: {
    getDetail(id) {
      fetch{{upperFirst modelName}}(id).then(response => {
        this.temp = response.data.data
{{#forOf attrs}}{{#compare ../selectedAttrs 'includes' k}}{{#compare v.UIType '===' 'remoteSelect'}}        this.{{rmId k}}s = [this.temp.{{rmId k}}]
{{/compare}}{{/compare}}{{/forOf}}      })
    },
...
{{/case}}{{/switch}}{{/compare}}{{/forOf}}    handleClose(updated) {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          const tempData = Object.assign({}, pick(this.temp, {{{printStringArray selectedAttrs}}}))
          tempData.timestamp = +new Date(tempData.timestamp)
          update{{upperFirst modelName}}(this.$route.params.id, cleanObject(tempData)).then(() => {
            this.$notify({
              title: '成功',
              message: '更新{{modelCnName}}成功',
              type: 'success',
              duration: 2000
            })
