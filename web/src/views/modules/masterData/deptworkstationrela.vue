  ...
  <div class="gen-list-page">
    ...
    <el-card class="with-title">
      <div slot="header" class="clearfix">
        ...
        <div class="buttons">
          <el-button v-if="isAuth('masterData:deptworkstationrela:create')" type="primary" @click="addOrUpdateHandle()">新增</el-button>

          <el-button v-if="isAuth('masterData:deptworkstationrela:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
        </div>
      </div>
      <el-table
        :data="dataList"
      ...
      <el-table-column align="center" :label="'操作'" width="230" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button v-if="isAuth('masterData:deptworkstationrela:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
            <el-button v-if="isAuth('masterData:deptworkstationrela:delete')" size="mini" type="text" @click="deleteHandle(scope.row)">删除</el-button>
          </template>
        </el-table-column>
...
<script>
import { listDeptWorkstationRela, deleteDeptWorkstationRela } from '@/api/deptWorkstationRela'
export default {
  name: 'deptWorkstationRelaList',
  data () {
    return {
      dataButton: 'list',
      listQuery: {
  ...
  methods: {
    ...
    getDataList (pageNo) {
      ...
      listDeptWorkstationRela(Object.assign(
        {
          page: this.pageNo,
          limit: this.pageSize
        },
    ...
    addOrUpdateHandle (id) {
      this.$nextTick(() => {
        this.$router.push({ path: id ? `/edit-deptworkstationrela/${id}` : '/add-deptworkstationrela' })
      })
    },
    // 删除数据
    deleteHandle (row) {
      ...
      this.$confirm('此操作将删除数据, 是否继续?', '提示', {
        ...
        deleteDeptWorkstationRela(ids).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
