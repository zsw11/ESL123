  ...
  <div class="gen-list-page">
    ...
    <el-card class="with-title">
      ...
      <el-table-column align="center" :label="'操作'" width="230" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button v-if="isAuth('masterData:modelseries:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
...
export default {
  ...
  methods: {
    ...
    addOrUpdateHandle (id) {
      this.$nextTick(() => {
        this.$router.push({ path: id ? `/edit-modelseries/${id}` : '/add-modelseries' })
      })
    },
    // 删除数据
    deleteHandle (row) {
