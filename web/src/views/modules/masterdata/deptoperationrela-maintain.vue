  ...
  <el-card class="with-title">
    ...
    <el-form :rules="dataRules" ref="dataForm" :model="dataForm" label-position="right" :size="'mini'" label-width="100px" style='width: 95%'>
          ...
          <el-form-item :label="'关键词ID'" prop="operationId">
            <el-input-number v-model="dataForm.operationId" ></el-input-number>
          </el-form-item>
...
import { fetchDeptOperationRela, createDeptOperationRela, updateDeptOperationRela } from '@/api/deptOperationRela'
export default {
  name: 'editDeptOperationRela',
  data () {
    return {
      inited: false,
      dataForm: {
      ...
      dataRules: {
        ...
        operationId: [
          { type: 'number', message: '关键词ID需为数字值' }
        ],
        createBy: [
  ...
  methods: {
    init () {
      ...
      if (this.dataForm.id) {
        fetchDeptOperationRela(this.dataForm.id).then(({data}) => {
          Object.assign(
            this.dataForm,
            pick(data, [ 'deptId', 'operationId', 'createBy', 'createAt', 'updateBy', 'updateAt', 'deleteAt' ])
          )
        }).finally(() => {
          this.inited = true
        })
    ...
    cancleFormSubmit () {
      ...
      this.$router.push({ name: 'masterData-deptoperationrela' })
      this.$destroy()
    },
    // 表单提交
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          (this.dataForm.id
            ? updateDeptOperationRela(this.dataForm.id, this.dataForm)
            : createDeptOperationRela(this.dataForm)
          ).then(({data}) => {
            this.$message({
              message: '操作成功',
              type: 'success',
