    ...
    <el-card class="with-title">
      ...
      <el-table-column align="center" fixed="right" :label="'操作'" width="230" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            ...
            <el-button  size="mini" type="text" class="search-box" @click="deleteHandle(scope.row)">删除</el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    details (id) {
      // let noShow = true
      this.$nextTick(() => {
        this.$router.push({path: `/details-workstationtype/${id}`, query: {noShow: true}})
      })
    },
    // 新增 / 修改
